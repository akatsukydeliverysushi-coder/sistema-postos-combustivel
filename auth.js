const AUTH_STYLE = `
#auth-screen{position:fixed;inset:0;z-index:9999;background:linear-gradient(135deg,#111827,#1f2937);display:flex;align-items:center;justify-content:center;padding:20px;overflow:auto}
.auth-card{width:min(430px,100%);background:#fff;border-radius:18px;padding:28px;box-shadow:0 20px 60px rgba(0,0,0,.35)}
...
function authEmailFromPhone(phone){
  ...
  return `${digits}@gerente.sistema-postos.local`;
}
function showAuth(){
  let old=document.getElementById('auth-screen'); if(old)old.remove();
  const box=document.createElement('div');box.id='auth-screen';
  box.innerHTML=`<div class="auth-card"><h2>⛽ Sistema de Postos</h2><p>Acesso seguro ao sistema</p><div class="auth-tabs"><button id="tab-adm" class="active">ADM</button><button id="tab-ger">Gerente</button></div><form id="auth-form"><div class="auth-field"><label id="auth-user-label">E-mail</label><input id="auth-user" required autocomplete="username" placeholder="administrador@email.com"></div><div class="auth-field"><label>Senha</label><input id="auth-pass" type="password" required minlength="6" autocomplete="current-password" placeholder="Mínimo de 6 caracteres"></div><div id="auth-error" class="auth-error"></div><button class="auth-submit" type="submit">Entrar</button><button id="btn-primeiro-adm" class="auth-secondary" type="button">Criar primeiro ADM</button></form></div>`;
  document.body.appendChild(box);
  let mode='adm';
  const user=box.querySelector('#auth-user'),label=box.querySelector('#auth-user-label');
  const tabAdm=box.querySelector('#tab-adm'),tabGer=box.querySelector('#tab-ger');
  function setMode(m){mode=m;const adm=m==='adm';tabAdm.classList.toggle('active',adm);tabGer.classList.toggle('active',!adm);label.textContent=adm?'E-mail':'Telefone';user.type=adm?'email':'tel';user.placeholder=adm?'administrador@email.com':'(19) 99999-9999';user.value='';}
  tabAdm.onclick=()=>setMode('adm');tabGer.onclick=()=>setMode('gerente');
  box.querySelector('#btn-primeiro-adm').onclick=async()=>{const email=prompt('Digite o e-mail do primeiro ADM:');if(!email)return;const senha=prompt('Crie uma senha para o ADM (mínimo 6 caracteres):');if(!senha)return;try{const cred=await firebaseAuth.createUserWithEmailAndPassword(email.trim(),senha);await firebaseDb.collection('users').doc(cred.user.uid).set({role:'admin',nome:'Administrador',ativo:true,email:email.trim()},{merge:true});alert('ADM criado. Você já está conectado.');}catch(e){alert(authMessage(e));}};
...
function authMessage(e){const c=e?.code||'';return ({'auth/invalid-credential':'Telefone/e-mail ou senha incorretos.','auth/user-not-found':'Usuário não encontrado.','auth/wrong-password':'Senha incorreta.','auth/email-already-in-use':'Este usuário já está cadastrado.','auth/weak-password':'A senha precisa ter pelo menos 6 caracteres.','auth/too-many-requests':'Muitas tentativas. Aguarde alguns minutos.'}[c])||'Não foi possível entrar. Confira os dados e tente novamente.';}

async function ensureUserProfile(user){
  const ref=firebaseDb.collection('users').doc(user.uid);const snap=await ref.get();
  if(!snap.exists){
    // O primeiro usuário criado pelo botão acima já recebe role admin.
    await ref.set({role:'admin',nome:user.email||'Administrador',ativo:true,email:user.email||''},{merge:true});
  }
...
firebaseAuth.onAuthStateChanged(async user=>{
  ...
  }catch(e){console.error(e);await firebaseAuth.signOut();alert('Não foi possível carregar o perfil de acesso.');}
});
