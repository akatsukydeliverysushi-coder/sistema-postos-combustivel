...
async function ensureUserProfile(user){
 ...
 if(!snap.exists){await ref.set({role:'admin',nome:user.email||'Administrador',ativo:true,email:user.email||''},{merge:true});}
 const data=(await ref.get()).data()||{};return data;
}
...
firebaseAuth.onAuthStateChanged(async user=>{
 ...
 }catch(e){console.error('Erro ao carregar perfil:',e);await firebaseAuth.signOut();const a=document.getElementById('auth-screen');if(!a)showAuth();const er=document.getElementById('auth-error');if(er){er.textContent='Login realizado, mas não foi possível carregar o perfil. Verifique o Firestore.';er.style.display='block';}}
});
